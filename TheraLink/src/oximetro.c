...
#define REG_LED1_PA             0x0C   // LED1 = RED (visível)
#define REG_LED2_PA             0x0D   // LED2 = IR  (invisível)
...
#define MODE_SPO2               0x03   // usa RED+IR (visible red aceso)

// ---------- Parâmetros “reais” ----------
#define RED_LED_CURRENT         0x24  // ~7–8mA (ajuste se precisar)
#define IR_LED_CURRENT          0x24  // ~7–8mA
#define FIFO_SMP_AVG_4          (0x02 << 5)  // média 4
#define FIFO_ROLLOVER_EN        0x10
#define FIFO_SMP_32             0x0F         // quase cheio
// SPO2: SR=100Hz (0b011), ADC=18-bit (0b11), LED_PW=411us
#define SPO2_CFG_100HZ_18BIT    0x2F

// Limiar para “dedo presente” (média simples do nível DC)
#define IR_FINGER_THRESHOLD     20000u

// Tempo de estabilização real em SETTLE (após detectar dedo)
#define SETTLE_REAL_MS          2500u  // mais tempo no "Calibrando..."

// ---------- Simulação (apenas no RUN) ----------
#define DEMO_MIN_BPM            87.0f
#define DEMO_MAX_BPM            92.0f
#define DEMO_TARGET_BEATS       8
#define DEMO_STEP_MS            2000u     // 1 valor a cada 2s
...
static inline uint32_t read_sample_18b(void) {
    // Lê 3 bytes e monta 18 bits (os 6 LSB são lixo)
    uint8_t b[3];
...
static bool max3010x_config_spo2(void) {
    // Garante desligado e zera FIFO
    if (!wr1(REG_MODE_CONFIG, MODE_SHUTDOWN)) return false;
    wr1(REG_FIFO_WR_PTR, 0);
    wr1(REG_OVF_COUNTER, 0);
    wr1(REG_FIFO_RD_PTR, 0);

    // FIFO: média, rollover, amostras
    if (!wr1(REG_FIFO_CONFIG, FIFO_SMP_AVG_4 | FIFO_ROLLOVER_EN | FIFO_SMP_32)) return false;

    // SPO2 config (SR e resolução)
    if (!wr1(REG_SPO2_CONFIG, SPO2_CFG_100HZ_18BIT)) return false;

    // Liga LEDs (vermelho visível + IR)
    if (!wr1(REG_LED1_PA, RED_LED_CURRENT)) return false; // RED visível
    if (!wr1(REG_LED2_PA, IR_LED_CURRENT )) return false; // IR

    // SPO2 mode (liga LED vermelho visível)
...
// Mede “dedo presente”: média simples do nível DC nas próximas N leituras
...
bool oxi_init(i2c_inst_t *i2c, uint sda, uint scl) {
    ...
    // Aceita uma faixa comum de IDs (alguns clones variam)
...
void oxi_start(void) {
    // Reconfigura (liga LED vermelho) sempre que começar uma medição
...
void oxi_poll(uint32_t now_ms) {
    ...
    case OXI_WAIT_FINGER: {
        // LED vermelho fica ligado (modo SpO2). Espera dedo real.
    ...
    case OXI_SETTLE: {
        // Mantém sensor ativo e FIFO sendo lido; dá tempo real para assentar
        (void)finger_present_check(12);
        // Se tirar o dedo durante a calibração, volta para WAIT_FINGER
        ...
        if (now_ms - s_t0_ms >= SETTLE_REAL_MS) {
            s_state = OXI_RUN;
            s_t0_ms = now_ms;
            s_last_step_ms = now_ms; // primeira leitura só após DEMO_STEP_MS
            s_valid = 0;
            s_live  = 0.0f;
            s_sum   = 0.0f;
            s_cnt   = 0;
    ...
    case OXI_RUN: {
        // Se tirar o dedo, interrompe e volta para WAIT_FINGER (zera progresso)
        if (!finger_present_check(6)) {
            s_state = OXI_WAIT_FINGER;
            s_t0_ms = now_ms;
            s_valid = 0;
        ...
        // Gera 1 valor a cada 2 segundos (simulado)
